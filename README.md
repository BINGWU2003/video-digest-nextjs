...
Docker Compose 默认使用阿里云 Debian APT 镜像源构建 worker，避免在国内网络下长时间停在 `deb.debian.org` 下载 `ffmpeg` 相关 Debian 包。直接执行：
...
如果需要换回官方源或其他镜像源，可以覆盖 `APT_MIRROR` 和 `APT_SECURITY_MIRROR`。这些变量只影响镜像构建阶段，不需要写入 `.env.docker`。
